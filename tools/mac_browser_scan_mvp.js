...
function pickInspectablePage(targets) {
...
function extractBlocksOnPage(fallbackPageUrl) {
  ...
  const extractFacebookPostUrlFromNode = (node) => {
    ...
    for (const anchor of anchors) {
      ...
      if (!existing || score > existing.score) {
        candidatesByUrl.set(normalized, { url: normalized, score });
      }
    }
  ...
  for (const selector of candidateSelectors) {
    ...
    for (const node of nodes) {
      ...
      results.push({
        text,
        postUrl: isFacebook ? extractFacebookPostUrlFromNode(node) : fallbackPageUrl,
      });
    }
    if (results.length >= 10) break;
  }
...
async function main() {
  ...
  try {
    ...
    target = pickInspectablePage(targets);
  } catch (error) {
    console.error(`Failed to query Chrome DevTools targets at ${cdpUrl}`);
    console.error('Make sure Chrome is running on your Mac with remote debugging enabled.');
    console.error(String(error));
    process.exit(1);
  }

  if (!target) {
    console.error('Connected to Chrome, but no inspectable page tabs were found.');
    process.exit(1);
  }

  const fallbackPageUrl = target.url || 'about:blank';
  const expression = `(${extractBlocksOnPage.toString()})(${JSON.stringify(fallbackPageUrl)})`;

  let pageState;
  try {
    pageState = await evaluateOnPage(target, expression);
  } catch (error) {
    console.error(`Failed to evaluate the page via Chrome DevTools at ${cdpUrl}`);
    console.error(String(error));
    process.exit(1);
  }

  const pageTitle = pageState?.pageTitle || target.title || '';
  ...
  const classified = blocks.map((block) => ({
    ...classifyBlock(block.text),
    postUrl: block.postUrl,
  }));
  const filtered = classified.filter((block) => block.isTattooRelated || block.intentScore >= 5);
  const leads = filtered.map((block) => buildLeadObject(block, pageTitle, pageUrl));

  fs.writeFileSync(outputPath, JSON.stringify(leads, null, 2));

  console.log(JSON.stringify(leads, null, 2));
  console.log(`\nWrote ${leads.length} lead object(s) to ${outputPath}`);
}
