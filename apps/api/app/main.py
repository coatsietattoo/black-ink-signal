...
class _PostedBrowserItem:
    ...
    def subreddit(self) -> str:
        return self.forum or self.raw.get("raw_page_title", "") or "browser_conversation"
...
class BrowserIngestItem(BaseModel):
    source: str
    item_id: str
    url: str
...
class BrowserIngestRequest(BaseModel):
    items: list[BrowserIngestItem]


def _parse_optional_iso_datetime(value: Optional[str]) -> datetime | None:
    if not value:
        return None
    try:
...
def _to_posted_browser_item(item: BrowserIngestItem) -> _PostedBrowserItem:
    return _PostedBrowserItem(
        source=item.source,
        ...
        forum=item.forum or "",
        created_at=_parse_optional_iso_datetime(item.created_at),
        raw=item.raw or {},
    )
...
@app.post("/admin/ingest-browser-items")
def admin_ingest_browser_items(body: BrowserIngestRequest):
    """Ingest normalized browser-derived items into the existing feed."""
    try:
        items = [_to_posted_browser_item(item) for item in body.items]
        with _Session() as session:
            run = SourceRun(source="browser_conversation")
            session.add(run)
            session.commit()
